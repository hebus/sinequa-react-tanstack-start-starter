# Démo — Application Sinequa avec TanStack Start (guide complet)

> Ce fichier reprend `PROMPT.md` (scaffolding + authentification) **et y ajoute** tout ce qui
> a été construit ensuite : recherche avancée (facettes, agrégations, tri, onglets, scopes,
> facettes arbre), preview de documents avec **surlignage avancé**, et **gestion de
> l'expiration de session**. Les sujets avancés ont aussi un tutoriel dédié dans
> `tutorial/aller-plus-loin/` (chapitres 1 à 4).

Guide reproductible (humain ou IA) pour construire une application **React / TanStack
Start** qui s'authentifie sur un backend **Sinequa** via `@sinequa/atomic`, puis :

- propose **trois méthodes d'authentification** : login/mot de passe (JWT via
  `api/v1/security.webtoken`), **Bearer token**, **SSO** SAML/OAuth ;
- expose des **routes protégées** (accueil, profil, recherche) derrière une garde unique ;
- branche une **recherche** Sinequa : pagination, **facettes/agrégations** (filtrage
  `query.filters`, « Voir plus », **facettes arbre**), **onglets**, **tri**, **scopes** ;
- ouvre la **preview** d'un document (sheet + iframe) avec **métadonnées** et **surlignage
  avancé** par catégorie (`highlightsPerLocation`) ;
- gère l'**expiration de session** (rafraîchissement auto + reconnexion/redirection) ;
- propose un **thème clair/sombre** persistant.
...
### 4.9 Recherche avancée — facettes, agrégations, tri, onglets, scopes

`/search` exploite tout ce que renvoie `Result` (`aggregations`, `tabs`, `sort`) et les
leviers de `Query` (`filters`, `tab`, `sort`, `scope`). Détail complet :
**`tutorial/aller-plus-loin/` → chapitre 2**. Points clés implémentés dans `search.tsx` :

- **Point d'entrée unique `execute(overrides)`** : fusionne l'état courant
  (`text/filters/tab/sort/scope/page`), **repart en page 1** (sauf pagination), persiste, et
  rappelle `fetchQuery`. Toute interaction (facette, onglet, tri, scope) passe par là.
- **Filtrage via `query.filters`** (API moderne, ≠ ancien `query.select`). On construit des
  objets `Filter` ; valeurs d'un même champ → `{ field, operator:'in', values }`, combinées
  en `{ operator:'and', filters }` (`ExprFilter`).

```tsx
function buildFilters(leaves: { field: string; value: string }[]): ExprFilter | undefined {
  if (leaves.length === 0) return undefined
  const byField = new Map<string, string[]>()
  for (const l of leaves) byField.set(l.field, [...(byField.get(l.field) ?? []), l.value])
  const perField: Filter[] = [...byField].map(([field, values]) =>
    values.length > 1 ? ({ field, operator: 'in', values } as InFilter) : { field, value: values[0] },
  )
  return { operator: 'and', filters: perField }
}
```

- **Facettes arbre** (`agg.isTree`) :
  - valeur de filtre = motif de chemin **`/<chemin>/*`** (et non la valeur brute) ;
  - **sélection unique** (un seul nœud par champ : cocher remplace) ;
  - **affichage hiérarchique** (rendu récursif, déplier/replier) avec **chargement paresseux**
    des enfants via l'action `open` — l'expression doit être **entre backticks** :

```tsx
open: [{ aggregation: agg.name, expression: `${agg.column}:\`/${path}/*\`` }]
// la réponse renvoie l'arbre depuis la racine ; on retrouve le nœud par $path et on greffe ses items.
```

- **« Voir plus »** d'une facette liste = action **`aggregate`** (ne pas confondre avec
  `fetchAggregation()`, qui sert à *récupérer* une agrégation) :

```tsx
const q: Query = { ...currentQuery, action: 'aggregate', aggregations: { [agg.name]: { skip, count } } }
const res = await fetchQuery(q) // on AJOUTE res.aggregations[name].items aux items existants
```

- **Onglets** : `result.tabs` → `query.tab`. **Tri** : `CCSortingChoice` (de
  `app.queries[name]`) → `query.sort`. **Scope** : `CCScope` → `query.scope`.

### 4.10 Preview — surlignage avancé (`highlightsPerLocation`)

En plus de l'iframe (§4.8), `fetchPreview()` renvoie des **données de surlignage** :
`highlightsPerCategory` (catégories + valeurs) et `highlightsPerLocation` (occurrences).
Détail complet : **chapitre 1** de `tutorial/aller-plus-loin/`. Dans `src/components/preview.tsx` :

- **Légende + compteur** par catégorie (depuis les données, sans toucher au document).
- **Coloration + navigation dans l'iframe** : l'iframe étant **same-origin** (proxy), on lit
  `iframe.contentDocument`. Les marqueurs portent **leur catégorie en classe CSS** (= les clés
  de `highlightsPerCategory`, ex. `.company`, `.geo`, `.matchingpassages`) → sélecteur
  autoritaire. Repli heuristique (signature DOM la plus proche du total) sinon.
- **Par catégorie** : chaque catégorie a un **toggle afficher/masquer** et **sa propre paire
  de flèches ⟨ ⟩** (curseur indépendant, scroll vers l'occurrence).
- **Libellés** : `categoryDisplayLabel*` sont des **clés i18n** (`msg#metadata.…PluralLabel`) →
  on les mappe vers des libellés lisibles (`CAT_LABELS`), repli sur la clé capitalisée.
- Le panneau est **repliable** (`<details>/<summary>`, même animation que les métadonnées).

### 4.11 Expiration de session — `withReauth`

Le rafraîchissement nominal est **automatique** : la lib stocke le token renvoyé dans
l'en-tête `sinequa-jwt-refresh` à chaque réponse (session glissante). Il ne reste qu'à gérer
le **chemin d'échec (401)**. Détail : **chapitre 3** de `tutorial/aller-plus-loin/`.

`src/auth/with-reauth.ts` :
...
export async function withReauth<T>(call: () => Promise<T>): Promise<T> {
  try {
    ...
    try { fresh = await getCsrfToken() } catch { /* pas de session ambiante */ }
    if (fresh) return await call()        // retry unique
    clearSessionTokens()                  // émet 'authenticated' = false -> redirection /login
    throw err
  }
}
```

Les appels de données de `/search` (`fetchApp`, `fetchQuery`, `fetchPreview`) sont enveloppés
via `withReauth(() => …)`. Sur expiration : reconnexion silencieuse si possible, sinon retour
automatique à `/login` (via l'événement `'authenticated'` écouté par l'`AuthProvider`).

> **Déploiement** : le proxy Vite n'existe qu'en dev. En production, reproduire la topologie
> via un **reverse proxy same-origin** (recommandé) ou `backendUrl` + CORS. Voir le
> **chapitre 4** de `tutorial/aller-plus-loin/`.
...
- ✅ **Recherche avancée** (§4.9) : facettes/agrégations (filtrage `query.filters`, « Voir
  plus » `aggregate`), **facettes arbre** (affichage hiérarchique + `open` testé sur le serveur,
  sélection unique), onglets, tri, scopes. Build + typecheck OK ; ouverture de nœud d'arbre
  validée en réel.
- ✅ **Surlignage preview** (§4.10) : légende/compteur + coloration et navigation par
  catégorie validés en réel (599 occurrences, marqueurs détectés via les classes de catégorie).
- ✅ **Expiration de session** (§4.11) : `withReauth` câblé sur les appels de `/search`
  (build + typecheck) ; reconnexion/redirection à vérifier en laissant la session expirer.
