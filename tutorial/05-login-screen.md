# 5. L'écran de connexion (3 méthodes)

L'écran `/login` propose trois onglets : **Login/mot de passe**, **Bearer token**, **SSO**.
Il consomme le `useAuth()` du chapitre précédent.

## 5.1 `src/routes/login.tsx`
        ...
        {method === 'sso' && (
          <div>
            ...
            <input
              ...
              placeholder={initializing ? 'Détection…' : 'identity-dev'}
            />
            <button
              className="primary"
            ...
            {!initializing && !detectedProvider && (
              <p className="hint">
                ...
                identity-dev) ou vérifiez VITE_SINEQUA_APP dans le fichier .env.
              </p>
            )}
...
- Le helper **`run()`** centralise `busy`/`error` autour de n'importe quelle action de login.
- **Redirection post-login** : un `useEffect` surveille `authenticated` et navigue vers `/`
  dès qu'il passe à `true` (peu importe la méthode utilisée).
- **Champ SSO éditable** : pré-rempli avec le provider détecté (`providers.oauth`), mais
  **modifiable** — la valeur saisie est transmise à `loginWithSSO(provider, kind)`. Indispensable
  pour passer de `identity` à `identity-dev` (chapitre suivant).

## 5.3 Pièges

> 💡 **Pré-remplissage sans écraser l'utilisateur** : `setSsoProvider((current) => current || detected)`
> n'écrit la valeur détectée que si le champ est encore vide. Sans ce garde-fou, le bootstrap
> (asynchrone) écraserait une saisie en cours.

> ⚠️ **`mint_rnd` n'annonce pas le login/mot de passe** en pré-login (seul l'OAuth est
> exposé). Les onglets login/mdp et Bearer restent câblés et fonctionnels si le serveur
> l'accepte ; à valider avec un compte de test.

Place au chapitre le plus subtil : le **SSO OAuth via Keycloak**.
